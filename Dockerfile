...
RUN yum install -y epel-release && yum install -y python-pip crmsh && pip install https://github.com/mvdbeek/pacemaker-etcd/archive/master.zip
...
After=dbus.service' /usr/lib/systemd/system/pcsd.service && \
    systemctl enable pcsd pcsd_setup pcs_etcd
ENV MY_IP=""
